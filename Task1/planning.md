...
* добавлять реплики для API-сервисов. Будет не так сложно, потому что прямого
  взаимодействия между сервисами нет, нужны будут только прокси/`BFF`/`ApiGW`
...
В первую очередь надо попытаться уменьшить нагрузку на `MES API` малыми усилиями
(например, кэшированием) и добавить наблюдаемость, которая поможет
диагностировать причины зависания заказов.

Параллельно, можно начать процесс найма еще одного QA-инженера, чтобы разгрузить
первого и начать заниматься автоматизацией тестирования.

Еще, хорошо бы добавить репликацию для БД, потому что это можно быстро добавить
и улучшить отказоустойчивость системы.

Если составлять список в порядке приоритета, будет примерно так:
1. Добавить кэширование для запросов чтения в `MES API`, чтобы разгрузить единственный инстанс
2. Добавить трейсинг, в первую очередь на API (`SHOP` и `MES`), чтобы отслеживать
время работы конкретных запросов
3. Добавить логирование и сбор логов для основных сервисов, чтобы получить возможность
хотя бы ручного разбора кейсов с потерей заказов
4. Добавить базовый мониторинг на сервисы и БД, например, алерты на падение
сервисов, уровень загрузки серверов (особенно `API`) и тд
5. Настроить репликацию баз данных (и `SHOP`, и `MES`)
6. Инициировать найм еще одного QA-инженера в команду
7. Добавить `nginx` в качестве прокси перед API, там можно настроить базовый
rate-limiting, чтобы дополнительно уменьшить нагрузку, хоть и ценой ошибок
для некоторых пользователей системы
8. Если добавленные мониторинги все еще показывают перегрузку `API`, можно
заняться их масштабированием, т.к. уже есть `nginx`
9. Начать добавление расширенных мониторингов, например, на время нахождения
заказов в определенном статусе (чтобы проверить зависания), а также на
конкретные ручки, в первую очередь расчет стоимости заказа
10. Если ручка стоимости заказа перегружена, можно заняться выделением расчета
стоимости заказов в отдельный сервис, чтобы ее в будущем можно было легко
масштабировать и сделать асинхронной
11. Подумать про шардирование баз, если это необходимо
12. Прочие инициативы, указанные ранее

### Целевая архитектура через полгода

За полгода можно добавить кэширование, базовую наблюдаемость (трейсинг, логи,
простые мониторинги), а также репликацию для БД.

Кэширование позволит немного разгрузить `API`, не инвестируя в это много
ресурсов.

Трейсинг и логи дадут возможность хотя бы вручную разбирать кейсы с проблемными
заказами в системе и находить несогласованности в существующей системе.

Базовые мониторинги уменьшат время даунтайма сервисов, а также помогут в
диагностике.

Репликация БД повысит надежность и отказоустойчивость системы в целом,
т.к. базы данных участвуют почти во всех процессах.

**Если выбирать только 3 пункта**, можно настроить кэширование, трейсинг и
базовые мониторинги. Это даст хотя бы небольшой уровень наблюдаемости,
возможность диагностики проблем в системе, ну и разгрузить `API` с учетом
постоянного повышения нагрузки.
